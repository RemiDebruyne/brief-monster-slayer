function renderHp(entityHpElements, hp) {
...
function renderLogs(logsElement, actionMaker, action, result) {
    logsElement.insertAdjacentHTML('afterbegin', `<p class="${actionMaker.toLowerCase()}-log">${actionMaker.charAt(0).toUpperCase()}${actionMaker.slice(1)} ${action} the monster for ${result} </p>`);
}
